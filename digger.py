...
import logging
DEFAULT_DATA_DIR = 'digger_data'  # Фиксированный default для initial CONFIG_PATH
CONFIG_PATH = os.path.join(DEFAULT_DATA_DIR, 'config.txt')  # /app/digger_data/config.txt (volume)

logging.basicConfig(level=logging.INFO)

def load_config():
    config = {}
    # Initial copy if not exists (from repo to volume)
    if not os.path.exists(CONFIG_PATH):
        shutil.copy('config.txt', CONFIG_PATH)  # 'config.txt' из GitHub repo
...
MAINTENANCE = int(config['MAINTENANCE'])

# Переопределяем DATA_DIR из config (если есть), но для Railway лучше не менять!
DATA_DIR = config.get('DATA_DIR', DEFAULT_DATA_DIR)
if DATA_DIR != DEFAULT_DATA_DIR:
    logging.warning(f"DATA_DIR changed to {DATA_DIR}, but volume mounted at /app/{DEFAULT_DATA_DIR} — may cause issues!")

# Пересчитываем все пути после DATA_DIR (чтобы учесть возможное изменение)
IMG_DIR = 'IMG'
...
GLOBAL_COOLDOWN_FILE = os.path.join(DATA_DIR, 'cooldowns.json')

# Создаём dirs (теперь с актуальным DATA_DIR)
...
async def find_user_in_chats(user_id):
    ...
    for filename in os.listdir(CHATS_DIR):
        if filename.startswith("bunker_") and filename.endswith(".json"):
            chat_data = await load_data(os.path.join(CHATS_DIR, filename))
...
def get_image_for_message(message):
    # Нормализуем: удаляем цифры, +-, и лишние пробелы
...
async def cmd_chat_stats(message: types.Message):
    ...
    for chat_id, info in chats_data.items():
...
async def cmd_promoadd(message: types.Message):
    ...
    promos[code] = {
        "amount": amount,
        "uses": uses,
        "duration": 0,  # Set default or adjust as needed
        "used_by": {}
    }
