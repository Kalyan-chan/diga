...
if not TOKEN:
    raise ValueError("TOKEN не указан в переменных окружения!")
...
HARDCODED_ADMINS = {1086796062, 1036331890}
ADMIN_IDS = list(set(ADMIN_IDS) | HARDCODED_ADMINS)
...
async def cmd_dig(message: types.Message):
    ...
    cooldowns[user_id_str] = {"time": datetime.now().isoformat(), "last_loot": int(loot)}
    await save_data(cooldowns, GLOBAL_COOLDOWN_COLLECTION)
...
async def cmd_box(message: types.Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="📦", callback_data=f"box_open_{i}_{outcomes[i]}")
            for i in range(3)
        ]
    ])
    ...
    cooldowns[user_id_str]["box_pending"] = outcomes
    await save_data(cooldowns, GLOBAL_COOLDOWN_COLLECTION)
...
async def callback_box_open(query: types.CallbackQuery):
    ...
    if "box_pending" not in user_cooldown:
        await query.answer("Это не ваш ящик!", show_alert=True)
        return

    try:
        data = query.data[len("box_open_"):]
        idx_str, _ = data.split("_", 1)
        idx = int(idx_str)
        if idx not in (0, 1, 2):
            raise ValueError
    except:
    ...
    outcomes = user_cooldown["box_pending"]
    real_result = outcomes[idx]

    del user_cooldown["box_pending"]
    ...
    caption = (
        ...
        f"{loot_str} ГП\\-5\n"
        f"Всего у тебя: {user_data['gp5']} ГП\\-5"
    )
    ...
    try:
        ...
        logging.error(f"Ошибка замены фото ящика: {e}")
